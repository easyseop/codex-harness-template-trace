...
./init.sh /path/to/project
    ...
    │   → AGENTS.md (스택별 조건부 섹션 포함)
    │   → ARCHITECTURE_INVARIANTS.md (3-tier invariant 프리필)
    │
    ├─ [Step 5] docs/ 생성
    │   → code-convention.yaml, adr.yaml
    │
    ├─ [Step 6] .codex/settings.local.json 설치
    │
    ├─ [Step 7] 커맨드 & 에이전트 복사
    │   → .harness/codex-harness/commands/ (8개 .md)
    │   → .harness/codex-harness/agents/ (9개 .md)
    │
    ├─ [Step 8] 게이트 & 규칙 설치
...
| 도구 | 역할 |
|------|------|
| `AGENTS.md.hbs` | 레이어 정의 + 스택별 매핑 명시 |
...
설치 위치: `.codex/settings.local.json`
...
3. AGENTS.md                   (AI 에이전트 컨텍스트)
4. docs/code-convention.yaml   (코딩 컨벤션)
```

### AGENTS.md (자동 생성)

`templates/AGENTS.md.hbs`에서 스택별 조건부 생성:
...
│
├── examples/                            # 스택별 예시
│   ├── nextjs-fastapi/AGENTS.md
│   ├── nextjs-nestjs/AGENTS.md
│   ├── nextjs-django/AGENTS.md
│   └── python-only/AGENTS.md
│
├── pro/                                 # Pro 버전 (Python)
...
| `strict.json` | JSON | Codex 프로덕션 권한 |
| `standard.json` | JSON | Codex 일반 개발 권한 |
| `permissive.json` | JSON | Codex 프로토타이핑 권한 |
