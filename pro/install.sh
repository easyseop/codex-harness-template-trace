...
if [ ! -f "$TARGET/AGENTS.md" ] || [ ! -d "$TARGET/.harness" ]; then
...
header "Step 5: Installing workflow command references"
...
success "Installed $(ls "$HARNESS_DIR/commands/"*.md | wc -l | tr -d ' ') command references"

# ─── Install personas ────────────────────────────────────────────
header "Step 6: Installing persona references"
...
success "Installed $(ls "$HARNESS_DIR/agents/"*.md | wc -l | tr -d ' ') persona references"
