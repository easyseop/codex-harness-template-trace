...
|---------|------|--------|-----------|
| [**v2.6.0**](https://github.com/studioKjm/ai-harness-template/releases/tag/v2.6.0) | 2026-05-15 | `stable` ⭐ **recommended** | **Isolated AI Security Gate** — security agent fully isolated from coding agent, eliminating confirmation bias. Fresh Codex process, zero shared context. Blocks on critical/high.codex CLI (Pro/Max) first; ANTHROPIC_API_KEY fallback. (NON-BREAKING) |
...
| [**v2.1.0**](https://github.com/studioKjm/ai-harness-template/releases/tag/v2.1.0) | 2026-04-19 | `stable` | **Pair Mode** — AC complexity-based selective activation, Navigator as persistent Codex role (subagent when available, sequential persona fallback otherwise), Test Designer with worktree isolation, Mixed Mode (direct+pair), auto /review checkpoints. Based on PairCoder (ASE 2024) + AgentCoder. |
...
Open the **cloned harness directory** in Codex, then run:
...
- `check-security-ai` — Isolated AI security analysis. Fresh Codex session, zero coding-agent context, adversarial persona. Blocks on critical/high findings.

See [`gates/GATES.md`](./gates/GATES.md) for details.

### AI Security Gate (v2.6.0)

The AI Security Gate runs a **completely isolated** Codex process with zero shared context from the coding agent — eliminating confirmation bias in vulnerability detection.
...
- `medium` → Navigator plans + Driver implements (explicit request/review loop)
...
```
harness/
├── .codex-plugin/     # Plugin manifest (for /plugin install)
├── init.sh             # Lite installer
├── commands/           # 10 Ouroboros workflow command references
...
├── templates/          # AGENTS.md, ARCHITECTURE_INVARIANTS, CI workflow
