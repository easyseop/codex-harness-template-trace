...
| `check-security-ai.sh` | **Isolated AI security analysis** — semantic vuln detection via fresh Codex session with zero shared context from coding agent. Blocks on critical/high. | Requires `codex` CLI (Pro/Max) or `ANTHROPIC_API_KEY`; incurs API cost per scan |
