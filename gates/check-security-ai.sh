...
#
# ISOLATION PRINCIPLE:
#   Spawns a fresh isolated AI process with ZERO context from the coding agent.
...
#
# Requires:
#   HARNESS_AI_SECURITY_CMD command that reads a prompt from stdin and returns JSON.
...
check_prerequisites() {
  ...
  if [ -z "${HARNESS_AI_SECURITY_CMD:-}" ]; then
    warn "HARNESS_AI_SECURITY_CMD not set — skipping AI security gate."
    warn "Set it to an isolated Codex-compatible command that reads the prompt from stdin and returns JSON."
    exit 0
  fi
...
# HARNESS_AI_SECURITY_CMD must read the prompt from stdin and return strict JSON.
# No conversation history from the coding agent is carried over.
call_security_agent() {
  local bundle_file="$1"

  python3 - "$bundle_file" <<'PYEOF'
import sys, os, json, re, subprocess, shlex

bundle_file = sys.argv[1]
security_cmd = os.environ.get('HARNESS_AI_SECURITY_CMD', '')
...
if security_cmd:
    try:
        result = subprocess.run(
            shlex.split(security_cmd),
            input=full_prompt, capture_output=True, text=True, timeout=180
        )
        if result.returncode != 0 and result.stderr:
            raise RuntimeError(result.stderr.strip())
        print(parse_response(result.stdout))
    except Exception as e:
        sys.stderr.write(f"Security command failed: {e}\n")
        print(json.dumps({"findings": [], "summary": f"Error: {e}"}))
else:
    sys.stderr.write("HARNESS_AI_SECURITY_CMD not set\n")
...
lines = [
    ...
    "_Agent: isolated security agent (Codex-compatible command, zero shared context)_",
    ""
]
...
info "Security command : ${HARNESS_AI_SECURITY_CMD:-not configured}"
